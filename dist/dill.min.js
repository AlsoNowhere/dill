...
var renderAttributes = function(template){
    ...
    forEach(template.attributes,function(attribute){
        ...
        if (attribute.name.substr(0,1) === attributeSpecialCharacter) {
            ...
            else if (elementProperties.indexOf(name) > -1) {
                element[name] = value;
            }
            else {
...
var fireOnInsertedEvents = function(template){
    ...
    // template.component && console.log("oninserted: ", template);
...
var renderIf = function(template,index){
    ...
    // console.log("Values: ", newValue, template.if.initialValue);
...
var render = function(template,index){
    ...
    if (template.if) {
        ...
        // console.log("Render element: ", template, ifResult);
    }
...
var createComponent = function(template){
    ...
    forEach(element.attributes,function(attribute){
        if (isSurroundedBy(attribute.nodeValue,"'")) {
            template.data[attribute.nodeName] = attribute.nodeValue.substring(1,attribute.nodeValue.length-1);
        }
        else {
            Object.defineProperty(template.data,attribute.nodeName,{
                get: function(){
